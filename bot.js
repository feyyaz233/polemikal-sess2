...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join("\n")}`
    ...
    msg.channel.send(
      `**${serverQueue.songs[0].title}** Adlı müzik tekrar devam ettirilene kadar durduruldu!`
    );
    serverQueue.songs = [];
    ...
    if (serverQueue && !serverQueue.playing) {
      ...
      return msg.channel.sendEmbed(
        new Discord.RichEmbed()
          .setTitle("Müzik keyfi devam ediyor :)")
          .setColor("GREEN")
      );
    }
    return msg.channel.sendEmbed(
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    ...
    try {
      ...
      return msg.channel.sendEmbed(
        new Discord.RichEmbed()
          .setTitle(`Sistemde bir hata var! Hata: ${error}`)
          .setColor("RED")
      );
    }
  } else {
    ...
    return msg.channel.sendEmbed(
      new Discord.RichEmbed()
        .setTitle(`**${song.title}** adlı müzik sıraya eklendi!`)
        .setColor("GREEN")
    );
  }
  return undefined;
