...
if(rol.id === veri){
  const embed = new Discord.RichEmbed()
      .setDescription(`Başarıyla rütbeler arasında **${message.guild.roles.get(veri).name}** rolüne sahip rütbe silindi!`)
      .setColor("BLACK")
      .setFooter(client.user.username, client.user.avatarURL);
...
await db.delete(`roldavet1_${message.guild.id}`);
  ...
  return
   ...
   const embed = new Discord.RichEmbed()
      .setDescription(`Başarıyla rütbeler arasında **${message.guild.roles.get(veri2).name}** rolüne sahip rütbe silindi!`)
      .setColor("BLACK")
      .setFooter(client.user.username, client.user.avatarURL);
